...
int main() {
    ...
    while (gaming) {
                ...
                drawButtons(musica, display);                
                 ...
                 if (botaoMusica) {                     
                     ...
                     if (musicPlaying == true) {
                         musica->obj = al_load_bitmap("img/btn-musica.png");
                         musica->y = 10;
                     }
                 }
        ...
        if (evento.type == ALLEGRO_EVENT_KEY_DOWN) {
            if (evento.keyboard.keycode == ALLEGRO_KEY_ESCAPE && begin 
                    && highscoreScreen) {
                tecla = 1;
            }
        }
        ...
        if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
            ...
            if (comecar->tipo && highscore->tipo && musica->tipo) {
                if (begin = botaoSelecionado(comecar, evento)) {
                    TIPO_OPERACAO = 1;
                }
                else if (begin = botaoSelecionado(highscore, evento)) {
