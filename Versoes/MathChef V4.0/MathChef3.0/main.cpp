...
int main() {
    ...
    ALLEGRO_FONT* font = al_load_ttf_font("font/font-principal.ttf", 15, 0);
        ...
        if (evento.type == ALLEGRO_EVENT_TIMER) {
            if (!start) {
                ...
                for (int i = 0; i < QTD_FOOD; i++) {
                    ...
                    if (foods[i].atived) {
                        drawFoods(foods[i], display, config->color, font);
                    }
                }

                al_draw_textf(font, config->color, 700, 80, 0, "%d", operacao->number);
                al_draw_textf(font, config->color, 100, 80, 0, "%d", valueOp);

                if (foodsSelected == 2 && valueOp == operacao->number) {
                    operacao = createOperation(TIPO_OPERACAO);

                    foods[7] = createFoods(foods, TIPO_OPERACAO, WIDTH_TELA, WH_FOOD);
                    foods[7] = createNumbers(foods, operacao);
                    foodsSelected = 0;
                    valueOp = 0;
                }
            ...
            if (start) {
                for (int i = 0; i < 7; i++) {
                    if (evento.mouse.x >= foods[i].x && evento.mouse.x <= foods[i].x + WH_FOOD &&
                        evento.mouse.y >= foods[i].y && evento.mouse.y <= foods[i].y + WH_FOOD) {
                        foodsSelected++;
                        if (foodsSelected <= 2) {
